...
\begin{tikzpicture}[>=stealth, scale=1]
	\begin{axis}[xmin = -1, xmax= 5, ymin=-1, ymax=8, axis x line=middle, axis y line=middle, axis line style=<->,
...
\end{align*}
Donc, on procédant comme ça, de proche en proche, on déduit que 
\[ u_n = u_0 + n \cdot r . \]
\end{proof}
...
\[ S_N = \sum_{n=0}^N u_n = \frac{N+1}{2} \cdot (u_0 + u_N). \]
\end{prop}
...
\item On sait que $10^{-b} = \dfrac1{10^b}$, on en déduit le résultat.
\item Conséquence de (i) et (ii).
...
\dots
\end{align*}
En utilisant le point (ii), on étend le résultat à $x \in \Q$. On admet le résultat pour le cas général, $x \in \R$.
\end{enumerate}
\end{proof}
...
\[ \begin{array}{c c c c} \log_b : & ]0, \infty[ & \to & \R \\ & x & \mapsto & \log_b(x) \end{array} \]
\end{defn}
