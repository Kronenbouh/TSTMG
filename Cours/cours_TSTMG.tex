...
La fonction inverse est la fonction définie sur $\R$ privé de 0 par :
\[ \begin{array}{c c c}
\Re & \to & \R \\
x & \mapsto & \dfrac1{x}
\end{array} \] 
...
\begin{defn}[Limite]
On considère une fonction $f$ définie sur un intervalle de la forme $]a,b[$ avec $a$ et $b$ des nombres éventuellements infinis.
...
c'est-à-dire à la variation de la valeur de $f(x)$ pour une très petite variation autour de $x$.
...
Soit $f$ une fonction dérivable sur un intervalle $I$. On appelle fonction dérivée de $f$, et l'on note $f'$, la fonction qui,
à un point $x$ de $I$ associe la dérivée de $f$ au point $x$.
\end{rapl}

\begin{nota}
Pour une fonction $f$ on note $f'$ sa fonction dérivée.
\end{nota}

\begin{prop}[Dérivée et sens de variation]
Soit $f$ une fonction numérique et $I$ un intervalle inclus dans le domaine de définition de $f$, 
...
\section{Fonction inverse et polynômes}
...
\begin{ex}
La fonction $x \mapsto x^2 -3x$ est une fonction polynôme de degré 2.
\end{ex}
...
& = ax^3 + 3ax^2h + 3axh^2 + ah^3 + bx^2 + 2bxh + bh^2 +cx + ch  -ax^3 - bx^2 -cx \\
