...
\begin{enumerate}
  ...
  \item \begin{enumerate}
	...
	\item Pour $x = 150$ on trouve $y \approx 75$.
...
\begin{enumerate}
  ...
  \item \begin{enumerate}
	...
	\end{align*}
	Donc la droite $(\Delta)$ passe par $G(\bar{x}, \bar{y})$.
	\item Pour $x = 140$ on trouve $y \approx 47$.
	...
	\begin{center}
		\begin{tikzpicture}
		   \tkzTabInit{$x$ / 1 , $CA'(x)$ / 1, $CA(x)$ / 1.5}{$0$, $80$, $\infty$}
		   \tkzTabLine{, +, z, -,}
		   \tkzTabVar{-/ , +/, -/ }
		\end{tikzpicture}
	\end{center}
	
	La fonction atteint donc un maximum pour $x \approx 80$ ce qui correspond à un chiffre d'affaire :
	\begin{align*}
	CA(50) & \approx -2.5 \times (80)^2 + 397,3 \times 80  \\
	& \approx 13~615 \\
	\end{align*}
	Soit 15~784 \euro~pour un prix unitaire de 80 \euro.
 \end{enumerate}
}
...
\addplot[teal, only marks] 
table[x=x,y=y, col sep=comma] {data_DS6b2.csv};
\end{axis}
\end{tikzpicture}
\end{center}
\begin{enumerate}
